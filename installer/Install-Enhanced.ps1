...
$RepoOwner = 'AC-Tech-Pro-Oficial'
$RepoName = 'GoLiveBypassEnhanced'
$RepoBranch = 'enhanced/rtc-viewer-recovery-v1'
$DiscordNames = @('Discord', 'DiscordPTB', 'DiscordCanary')

function Resolve-EnhancedCommit {
    $inherited = [Environment]::GetEnvironmentVariable('GOLIVE_ENHANCED_REF')
    if ($inherited -match '^[0-9a-fA-F]{40}$') {
    ...
    try {
        ...
        throw "Nao consegui fixar a versao enhanced antes de instalar: $($_.Exception.Message)"
    }

    $sha = [string]$commit.sha
    if ($sha -notmatch '^[0-9a-fA-F]{40}$') {
        throw 'O GitHub nao devolveu um commit valido para a versao enhanced.'
    }
    return $sha.ToLowerInvariant()
}

$RepoRef = Resolve-EnhancedCommit
$env:GOLIVE_ENHANCED_REF = $RepoRef
$RepoRaw = "https://raw.githubusercontent.com/$RepoOwner/$RepoName/$RepoRef"
